...
class BaseSite(ABC):
    ...
    def checkout(self, url: str, profile: Dict, captcha_queue=None, api_data=None) -> Dict:
        ...
        try:
            ...
            add_result = self.add_to_cart()
            if not add_result:
                # Check if it was out of stock vs other failure
                result['message'] = 'Out of stock or failed to add to cart'
                result['out_of_stock'] = True  # Flag for monitoring
                print(f"[Checkout] ✗ {result['message']}")
                print("[Checkout] ⚠️  Item may be out of stock or unavailable")
                return result
